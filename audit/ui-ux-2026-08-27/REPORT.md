...
이 문서의 P1·P2 및 묶음 A–C 요구사항을 코드에 반영했다. 각 묶음은 독립 검증 뒤 별도 커밋했으며, 오류 복구처럼 같은 화면의 상태 모델과 UI를 함께 바꿔야 하는 항목은 기능 단위로 묶었다.

| 요구사항 | 구현 결과 | 주요 커밋 |
| --- | --- | --- |
| 공통 토큰·44pt·semantics·Dynamic Type | 의미 역할 색, 강한 outline, 공통 비동기 상태와 아이콘 버튼, 320/375pt·200/300% 재배치 기반을 적용 | `7a19ab7`, `0c226a3`, `551e128` |
| 실패 시 기존 콘텐츠 유지 | 검색어·결과, 미디어 그리드, 앨범 목록/사진, 알림 설정 draft를 유지하고 inline 오류·busy 재시도·live region을 분리 | `2403286`, `77bcfd1`, `f50f375`, `e39997d`, `9e5a7c5` |
| 홈·채팅 접근성 | 홈 우선순위와 큰 글자 배치, 44pt 헤더/작성 동작, 메시지 더보기 대체 경로, 반응 상태 semantics를 적용 | `0c226a3`, `fcb22d4`, `f78d212` |
| 사진 전체보기 | 현재 위치와 제스처 안내, 개별 재시도, 로딩/실패 중 저장·공유 잠금과 복구 가능한 상태를 구현 | `166ecbc`, `038993e` |
| 앨범 흐름 | 단순화한 생성 시트, 선택/취소 semantics, 부분 실패, pagination/realtime/mutation 갱신, 기본 커버 자산을 구현 | `9e5a7c5`, `1862a60` |
| 설정·위험 작업 | 로컬 설정 보존, 연결 해제/계정 삭제의 영향·복구 가능 여부·재확인, dark semantic surface를 적용 | `2f596ef`, `e39997d`, `a44f527` |
| 인증·페어링 | 이메일/비밀번호 persistent label, 오류 요약과 포커스, 4자리 입력의 현재 자릿수·붙여넣기 semantics를 구현 | `7a8723f`, `a88a583` |
| 알림 권한 | 자동 토큰 동기화와 권한 요청을 분리하고, 사전 설명·나중에·거절 후 설정 열기·단계별 복구 흐름을 구현 | `55de2fa`, `3a1bed0`, `d50abaf` |
| 시스템 다크 모드 | 완성된 light/dark `ColorScheme`와 `ThemeMode.system`을 적용하고 공통·홈·채팅·앨범·설정·검색·인증·기념일·지도·오목 surface를 의미 역할 색으로 전환 | `de056f6`, `551e128`, `1862a60`, `fcb22d4`, `a88a583`, `f78d212`, `a44f527`, `87e1193`, `ced4644`, `03061fb`, `d50abaf` |
| 오목 접근성 | 판 확대/팬, 교차점 탭 후 좌표 확인, 행·열 선택기와 48pt 돌 놓기 버튼, 점유/차례/종료 live 상태, bottom safe area를 구현 | `b530703`, `d50abaf` |
| 지도 접근성 | 캔버스 설명, 항상 접근 가능한 장소 목록, 가로·200% breakpoint와 safe area, 편집/탐색 dark 및 Reduce Motion을 구현 | `4b90fd4`, `03061fb` |
| 전환·Reduce Motion | adaptive/native route와 system back을 복원하고 route·지도·온보딩·오목 모션을 `disableAnimations`에 연결 | `be20ba1`, `03061fb`, `d50abaf` |
| 기념일·이미지 자산 | 작은 raster control glyph를 Material vector로 교체하고 Dear 로고/홈 이미지 decode 크기를 제한 | `17f6794`, `ced4644` |

### 자동 검증 결과

- `flutter analyze --no-pub`: 문제 없음.
- `flutter test --no-pub --reporter compact`: **290/290 통과**.
- 지도 묶음: 42/42 통과. 다크 지도, Reduce Motion, 375pt·200%, 가로·200%, 장소 목록 대체 경로를 포함한다.
- 온보딩·알림함·비밀번호 복구·오목 최종 묶음: 24/24 통과. 다크 모드, 즉시 전환, 좌표 착수, 마지막 수 표시의 보드 대비 3:1 이상을 포함한다.
- 앨범은 320pt·300%, 홈·채팅·설정·지도·오목은 320/375pt·200% 또는 가로모드 위젯 테스트로 핵심 overflow와 조작 경로를 회귀 검증했다.

### 배포 전 실기기 확인

자동 검증은 구현 회귀를 확인한 것이며, 최초 감사에서 확인하지 못한 실제 아이폰의 VoiceOver 읽기 순서·최대 접근성 글자·다크 모드·Reduce Motion·가로모드와 APNs 시스템 권한 프롬프트는 배포 전 수동 QA가 필요하다. 이는 미구현 항목이 아니라 OS·실기기 동작을 확인하는 최종 릴리스 게이트다.
